...
# Function to install selected packages on Debian-based systems
install_packages_debian() {
    sudo nala update
    sudo nala install -y "$@"
}

# Function to install selected packages on Arch-based systems
install_packages_arch() {
    echo "Installing selected packages for Arch-based system..."
    paru -S --noconfirm "$@"
}

# Function to handle package installation based on distribution
install_packages() {
...
# Utilities (added fastfetch)
utilities=( "gparted" "gnome-disk-utility" "neofetch" "fastfetch" "nitrogen" "numlockx" "galculator" "cpu-x" "udns-utils" "whois" "curl" "tree" "btop" "htop" "bat" "brightnessctl" "redshift" )
